...
import { AgentActionSchema, calculateRumorImpact, generateUUID } from '@wallstreetsim/utils';
import type { AgentAction, MarketEvent } from '@wallstreetsim/types';
...
interface ProcessedActionResult {
  ...
  /** Market events generated by actions (e.g., rumors) */
  ...
  for (const rawAction of actionsToProcess) {
    ...
    try {
      ...
      if (actionResult.success) {
        result.succeeded++;
        // Collect market events from successful actions
        if (actionResult.marketEvent) {
          result.marketEvents.push(actionResult.marketEvent);
        }
      } else {
        result.failed++;
      }
 ...
 *
 * Creates a MarketEvent that affects the target stock's price based on
 * the sentiment of the rumor content. Positive rumors boost price,
 * negative rumors decrease it.
 */
